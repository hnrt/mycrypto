...
%command% aes-256-gcm -e -i ..\plaintext44.txt -o ciphertext44.gcm -p %pp% -nonce %nonce12% -aad %aad%
...
%command% aes-256-gcm -d -i ciphertext44.gcm -o plaintext44-2.gcm -p %pp% -aad %aad%123
@IF ERRORLEVEL 1 (@ECHO OK for STATUS_AUTH_TAG_MISMATCH) ELSE (@EXIT /B 2)

%command% aes-256-gcm -e -i ..\plaintext6570.txt -o ciphertext6570.gcm -p %pp% -nonce %nonce12% -aad %aad%
...
%command% aes-256-gcm -e -i ..\plaintext2M.jpg -o ciphertext2M.gcm -p %pp% -nonce %nonce12% -aad %aad%
